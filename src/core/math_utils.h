#pragma once
// Operator overloads and small helpers on raylib's Vector2/Vector3/Matrix types.
// Header-only; everything is inline.
#include <cmath>
#include <set>
#include <vector>
#include "raylib.h"
...
inline void UpdateTransformPosition(Matrix& m, const Vector3& v) {
	m.m12 = v.x;
	m.m13 = v.y;
	m.m14 = v.z;
}

inline Vector3 PositionVectorFromTransform(const Matrix& m) {
	return (Vector3){m.m12, m.m13, m.m14};
}

inline Vector3 ScaleVectorFromTransform(const Matrix& m) {
	return (Vector3){norm(m.m0, m.m1, m.m2), norm(m.m4, m.m5, m.m6), norm(m.m8, m.m9, m.m10)};
}

inline Matrix RotationTransformFromTransform(const Matrix& m) {
...
inline Vector3 centroid(std::vector<Vector3> points) {
...
inline Vector3 centroid(std::vector<Vector3> points, std::set<int> indices) {
